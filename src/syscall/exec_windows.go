...
func StartProcess(argv0 string, argv []string, attr *ProcAttr) (pid int, handle uintptr, err error) {
	...
	// bit is not checked.
	isLegacyWinConsoleHandle := func(handle Handle) bool { return isLegacyWin && handle&0x10000003 == 3 }
	...
	for i := range attr.Files {
		if attr.Files[i] > 0 {
			...
			if parentProcess != p && isLegacyWinConsoleHandle(Handle(attr.Files[i])) {
	...
	for i := range fd {
		if isLegacyWinConsoleHandle(fd[i]) {
			fd[i] = 0
		}
	}
